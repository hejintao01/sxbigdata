    ...
    <div class="table">
      <div class="content">
        ...
        <div class="contenttitle" v-for="list in listdata" :key="list.pm">
          ...
          <span class="contentfont">{{list.fzl}}</span>
          <span class="contentfont">{{list.lrl}}</span>
          <span class="contentfont">{{list.xjl}}</span>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  data() {
    return {
      title: '代理机构经营能力分析',
      headerdata: ['当前排名','代理机构', '资产负债率','净利润率','现金流'],
      listdata: [
        {pm:'2', name: '机构A', fzl: '7.6',lrl:'8',xjl:'8'},
      ]
    }
  }
}
