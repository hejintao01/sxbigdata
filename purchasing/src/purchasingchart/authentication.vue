...
export default {
  ...
  data() {
    return {
      title: '岗位认证积分排名',
      headerdata: [ '排名', '编号',  '姓名','人员类型' ,'职位', '岗位认证得分',],
      listdata:null
    }
  },
  computed: {
    filterData: function () {
      console.log("岗位认证积分排名所用数据",JSON.parse(JSON.stringify(this.listdata)));
   ...
   created() {
    ...
    // 将yigo查询的值赋值给list
    let arr = window.parent.exec(formID, "DBNamedQuery('PositionScoreRanking')");
    console.log('岗位认证积分排名', arr);
    // 赋值
