    ...
    <div class="table">
      <div class="content">
        ...
        <div class="contenttitle" v-for="list in listdata" :key="list.pm">
          ...
          <span class="contentfont">{{list.cp}}</span>
          <span class="contentfont">{{list.name}}</span>
          <span class="contentfont">{{list.PF}}</span>  
          <span class="contentfont">{{list.wz}}</span>
          <span class="contentfont">{{list.YJ}}</span>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  data() {
    return {
      title: '产品质量预警',
      headerdata: ['排名','产品','供应商', '评分','位置','预警程度'],
      listdata: [
        {pm:'1', cp:'产品A',name: '供应商A', PF:'5.1',wz:'0-5%',YJ:''},
        {pm:'2',cp:'产品B', name: '供应商B', PF:'5.2',wz:'0-5%',YJ:''},
        {pm:'3',cp:'产品C', name: '供应商C', PF:'5.3',wz:'5-10%',YJ:''},
      ]
    }
  }
}
